...
ctest      DATA NEVMOD/1000/
      DATA NEVMOD/100/  !print event characteristic in outfile each nevmod
      SAVE IFLCNT
C
      IF(LFIRST) THEN
        LFIRST=.FALSE.
...
C...Print every 'nevmod'th event
            IF (MOD(NEVHEP,NEVMOD).EQ.0) THEN !print for an event each NEVMOD
...
C...write events to file
         ...
         gNU = (W2+Y-MPRO2)/(2.0*MPRO2)
         gNUHAD= (W2HAD+YHAD-MPRO2)/(2.0*MPRO2)
C         D=(S*S-U*U)/(S*S+U*U)
C         CALL HSFG(X,Q2,LLEPT)
C         write(31,32) 0, NEVHEP, ICHNN, LST(23), LST(24), LST(22),
C     &   LST(25), LST(26), Y, Q2, X, W2, gNU, YHAD, Q2HAD, XHAD, W2HAD,
C     &   gNUHAD, SIGTOT,SIGTRR, D,F1NC,F3NC,G1NC,G3NC,A1NC,
C     &   F1CC,F3CC,G1CC,G5CC, nrtrack
C 32      format((I4,1x),(I10,1x),6(I6,1x),22(E18.10,1x),I12)
         write(31,*) ' '
         write(31,*) 'Event N.',NEVHEP,'      Channel =',ICHNN
         write(31,32) 'X, Y, Nu , Q2, W2 =    ',
     &  X, ',' , Y, ',', gNU, ',', Q2, ',', W2
         write(31,32) 'True X, Y, Nu, Q2, W2 =', 
     &  XHAD, ',', YHAD, ',', gNUHAD, ',', Q2HAD, ',', W2HAD  
 32      format(5(A,E18.10,1x))
         write(31,*)'============================================'

         DO I=1,N
           if (K(I,3).le.N) then
             write(31,34) (I),K(I,1),K(I,2),K(I,3),K(I,4),K(I,5)
     &                    ,P(I,1),P(I,2),-1.*P(I,3),P(I,4),P(I,5)
C     &                    ,V(I,1),V(I,2),V(I,3)
           endif
         ENDDO
 34      format(6(I10,1x),5(f15.6,1x))
         write(31,*)'=============== Event finished ==============='
         write(31,*) ' '

C       WRITE(31,2301) ICHNN
C       WRITE(31,2302) IDHEP(1)
C       WRITE(31,2303) PHEP(1,1),PHEP(2,1),PHEP(3,1),PHEP(4,1),PHEP(5,1)
C       WRITE(31,2302) IDHEP(2)
C       WRITE(31,2303) PHEP(1,2),PHEP(2,2),PHEP(3,2),PHEP(4,2),PHEP(5,2)
C       WRITE(31,2302) IDHEP(3)
C       WRITE(31,2303) PHEP(1,3),PHEP(2,3),PHEP(3,3),PHEP(4,3),PHEP(5,3)
C 2301 FORMAT(I4)
C 2302 FORMAT(I4)
